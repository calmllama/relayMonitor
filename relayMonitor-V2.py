...
def mainThread (window):
    while True:
        ...
        for address, adc, analog, relay in zip(ADDRESSES, ADCs, ANALOGS, RELAYS):
            ...
            try:
                ...
                continue
            sock.settimeout(.5)
            # Cycle through each digital input. each loop is the next DI on the same board
            for x in adc:
                if x == 'EMPTY':
                    continue
                try:
            ...
            # Cycle through each analog input. each loop is the next AI on the same board
            for y in analog:
                if y == 'EMPTY':
                    continue
                try:
        ...
        for z in relay:
            ...
            try:
                # Get relay state
                value = get_relay_status_by_index(relay[z])
                # Handle rare cases of getting non-numeric data back. Just redo until you get good data
                while not str(value).isnumeric() and not z == "EMPTY":
                    value = get_relay_status_by_index(relay[z])
            except Exception as e:
                print (e)
                continue
            try:
                    ...
                    state = 2
                # Define data to write and trigger GUI event to write it
                data = [z, address, relay[z], state]
                window.write_event_value("-RELAY-THREAD-", data)
...
threading.Thread(target=mainThread, args=(window,), daemon=True).start()

# Function for toggling buttons
def buttonToggle(button, currentState):
    if currentState == 0:
        window[button].update(button_color=COLORS[0])
    elif currentState == 1:
        window[button].update(button_color=COLORS[1])
    else:
        window[button].update(button_color=COLORS[2])
...
while True:
    ...
    # Relay event to update texts
    elif event == "-RELAY-THREAD-":
        key, currentState = values[event]
        window[key].update(button_color=COLORS[currentState])
    ##################################################################### Button Events
    # Event for unlock button
    elif event == "UNLOCK RELAY BUTTONS":
        threading.Thread(target=buttonThread, args=(window,), daemon=True).start()
    ...
    # Individual relay buttons
        ...
        sendRelay ('192.168.10.31', 1)
    elif event == list(RELAYS[5])[0]:
        sendRelay ('192.168.10.36', 1)
    elif event == list(RELAYS[5])[1]:
        sendRelay ('192.168.10.36', 2)
    elif event == list(RELAYS[5])[2]:
        sendRelay ('192.168.10.36', 3)
    elif event == list(RELAYS[5])[3]:
        sendRelay ('192.168.10.36', 4)
    elif event == list(RELAYS[5])[4]:
        sendRelay ('192.168.10.36', 5)
    elif event == list(RELAYS[5])[5]:
        sendRelay ('192.168.10.36', 6)
    elif event == list(RELAYS[5])[6]:
        sendRelay ('192.168.10.36', 7)
    elif event == list(RELAYS[5])[7]:
        sendRelay ('192.168.10.36', 8)
    elif event == list(RELAYS[6])[0]:
        sendRelay ('192.168.10.37', 1)
    elif event == list(RELAYS[6])[1]:
        sendRelay ('192.168.10.37', 2)
    elif event == list(RELAYS[6])[2]:
        sendRelay ('192.168.10.37', 3)
    elif event == list(RELAYS[6])[3]:
        sendRelay ('192.168.10.37', 4)
window.close()